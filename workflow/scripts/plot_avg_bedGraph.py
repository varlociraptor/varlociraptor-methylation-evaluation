    ...
    with open(file_path, "r") as file:
        for line in file:
            ...
            if current_data is not None:
                positions_to_graphs[(chrom, pos)].append((i, coverage, methylation))
            else:
...
for (chrom, pos), values in positions_to_graphs.items():
    for i, coverage, methylation in values:
        data.append({'chrom': chrom, 'pos': pos, 'i': i, 'coverage': coverage, 'methylation': float(methylation)})
df = pd.DataFrame(data)
unique_positions = df['pos'].unique()
random_positions = random.sample(list(unique_positions), min(20, len(unique_positions)))
...
chart1 = alt.Chart(df).mark_circle().encode(
    x=alt.X('pos:O', title='Positions', axis=alt.Axis(labelAngle=0), sort=alt.EncodingSortField(field="mean_methylation", order='ascending')),
