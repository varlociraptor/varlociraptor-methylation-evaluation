...
df_merged.to_parquet(snakemake.output["protocol_df"], engine="pyarrow", compression="snappy")

# ---- Plots erstellen ----

# Densityplots
# methylation_density_chart = density_plot_agg(df_merged, tool_names, "methylation")
# coverage_density_chart = density_plot_agg(df_merged, tool_names, "coverage")

# # Distanceplot (voraggregiert)
# melted_data = compute_distances(df_merged, tool_names)
# agg_dist = melted_data.groupby(['Tool','Distance']).size().reset_index(name='count')
# distance_chart = distance_plot(agg_dist)

# # Charts kombinieren
# chart = methylation_density_chart | coverage_density_chart | distance_chart

# # Speichern als interaktive HTML
