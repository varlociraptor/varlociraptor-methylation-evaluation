...
rule compute_coverage:
    ...
    params:
        extra=lambda wildcards: (
            f"--no-per-base --use-median --mapq {wildcards.mapq}"
            if wildcards.mapq != "all"
            else ""
        ),
        # optional
    wrapper:
...
rule common_coverage_Illumina:
    input:
        lambda wildcards: expand(
            "results/single_sample/Illumina_pe/coverages/{sample}_{{REP}}_{{mapq}}.regions.bed.gz",
            sample=config["samples"]["Illumina_pe"],
        ),
    output:
...
rule compute_coverage_retention:
    ...
    output:
        mae=report(
            "results/{call_type}/{seq_platform}/plots/{sample}_dist_{mapq}.{plot_type}",
            category="{call_type}",
    ...
    params:
        ...
        quantile=lambda wildcards: (
            0.995 if wildcards.seq_platform == "Illumina_pe" else 0.95
        ),
        bin_size=5,
    script:
